...
| ¿Qué instrucción se recibió respecto a él? | **William indicó expresamente que Aether-Hermes debía retirarse.** | VERIFIED — instrucción expresa del usuario |
| ¿Se retiró? | **No.** Sigue presente en Graphy y en la topología actual | VERIFIED |
| Estado actual | Presente, con identidad procedente del master | VERIFIED |
| Incidencia | La instrucción de retirada **no quedó ejecutada** en el estado actual del producto | VERIFIED |
| Alcance exacto de la retirada | **No especificado en el material recuperado.** No se debe inferir si afecta solo a Graphy, a `graphy-data.js`, a las relaciones, al master o a varios artefactos | UNKNOWN |
| Contradicciones | El estado actual del producto contradice la instrucción de retirada del usuario | VERIFIED |

**Instrucción prioritaria para el siguiente agente:** Aether-Hermes debe considerarse una
**incidencia abierta de producto**, no una decisión desconocida. William ha indicado que quiere
retirarlo, pero el material recuperado no determina todavía el alcance técnico exacto. **No
inferir el alcance. No modificar el master ni `graphy-data.js` por iniciativa propia.** Si William
decide continuar con esta incidencia, deberá determinarse primero qué artefactos deben cambiar.
...
### 24.1 ESTADO DE PRODUCTO VISUAL — DISTINCIÓN NECESARIA

Graphy 1.0 está **validado funcionalmente**, pero este expediente no certifica que su
productización visual sea el nivel final esperado para una pieza de presentación.

Estado documentado:

- **Funcional:** validado.
- **Arquitectura:** documentada y congelada.
- **Interacción:** validada.
- **Determinismo:** validado.
- **Editabilidad:** validada.
- **Independencia del MVP:** validada.
- **Dirección visual:** implementada en la pasada de productización (luz, retícula, órbitas,
  monogramas, emblema, jerarquía y transiciones).
- **Identidad visual individual de las Presence:** basada actualmente en monogramas; no se ha
  incorporado material visual individual.
- **Adecuación al nivel visual final esperado:** **NO CERTIFICADA** por este expediente.

La finalidad de esta distinción es evitar que «validado técnicamente» se interprete como
«producto visual definitivo terminado».
...
### 26.1 Estado entregado — GRAPHY 1.0

| Artefacto | Estado |
| --- | --- |
| Graphy 1.0 (`Graphy 0.1.dc.html`) | **IMPLEMENTADO · VALIDADO · CERRADO · CONGELADO** |
| `graphy-data.js` | **CONGELADO** para la versión 1.0 |
...
### 26.2 Trabajo post-1.0 — NO AUTORIZADO

Estas cuestiones quedan documentadas como posibles trabajos posteriores, pero **no forman parte
de Graphy 1.0 y no autorizan modificaciones por sí mismas**:

1. Retirada de Aether-Hermes: instrucción confirmada; alcance técnico pendiente.
2. Fijación histórica de `updatedAtSimulated`.
3. Renombrado del archivo y actualización de rótulos de versión.
4. Desacoplamiento de la escala de etiquetas respecto al `padding-bottom:64%`.
5. Unificación del criterio de acento con el master.
6. Sustitución de monogramas por material visual real.
7. Ajustes visuales adicionales del mapa o de las Presence.

**Regla de continuidad:** una cuestión incluida aquí requiere una nueva orden expresa antes de
modificar cualquier artefacto congelado.
...
| 14 | Aether-Hermes permanece pese a la instrucción de retirada | La instrucción no fue ejecutada y el alcance no está determinado | **Incidencia abierta; no modificar hasta definir alcance** |
...
Graphy 1.0 está **implementado, validado, cerrado y congelado** como versión funcional.

Es la capa visual y relacional del ecosistema WAIPL: recorre las relaciones **declaradas** del
ecosistema sin ejecutar el laboratorio.

Existe **una incidencia abierta de producto**: William ha indicado que Aether-Hermes debe
retirarse, pero el estado actual todavía lo contiene. El alcance técnico exacto de esa retirada
no debe inferirse ni ejecutarse automáticamente.

Fuera de esa incidencia, no hay fallos funcionales bloqueantes documentados en Graphy 1.0.
...
**No modificar nada al recibir este expediente.**

1. Leer el expediente completo.
2. Tratar Graphy 1.0 como versión cerrada y congelada.
3. Registrar Aether-Hermes como incidencia abierta con instrucción expresa de retirada.
4. **No ejecutar la retirada hasta determinar su alcance exacto.**
5. Si William autoriza una nueva fase, reproducir primero el estado de referencia y trabajar
   exclusivamente sobre el alcance de esa nueva orden.

---

## 33. NOTA DE TRANSFERENCIA FINAL

Este expediente describe el estado técnico y funcional de Graphy al cierre de la versión 1.0.
La transferencia no debe reiniciar el desarrollo ni reinterpretar decisiones ya validadas.

**Reglas de continuidad:**

- Lo que figura como **CERRADO / CONGELADO** no se reabre sin orden expresa.
- Lo que figura como **NO AUTORIZADO** no se ejecuta por iniciativa propia.
- Las relaciones se consideran datos declarados; no deben inventarse ni derivarse por conveniencia.
- El master es fuente de identidad y se trata como solo lectura salvo autorización expresa.
- El MVP, motor y configuraciones permanecen fuera del alcance de Graphy.
- Las incidencias abiertas deben conservar su trazabilidad y no convertirse silenciosamente en
  nuevas decisiones de arquitectura.
- La retirada de Aether-Hermes es una **instrucción real del usuario**; lo único pendiente es
  determinar su alcance técnico.

---

*Fin del expediente. Ningún archivo del producto ha sido modificado durante la redacción de esta
versión corregida del expediente.*
