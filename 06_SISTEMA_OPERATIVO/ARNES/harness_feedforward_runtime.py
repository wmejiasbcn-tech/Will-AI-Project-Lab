...
def consume_feedforward() -> dict[str, object]:
    if not PAYLOAD_PATH.is_file() or PAYLOAD_PATH.is_symlink(): raise FileNotFoundError(f"Invalid feedforward payload: {PAYLOAD_PATH}")
    payload = json.loads(PAYLOAD_PATH.read_text(encoding="utf-8"))
    missing = REQUIRED_KEYS.difference(payload)
    if missing: raise ValueError(f"Missing feedforward keys: {sorted(missing)}")
    content = payload["guide_content"]
    if not isinstance(content, str) or not content.strip(): raise ValueError("Feedforward guide_content must be non-empty text")
    if hashlib.sha256(content.encode("utf-8")).hexdigest() != payload["sha256"]: raise ValueError("Feedforward SHA-256 does not match guide_content")
